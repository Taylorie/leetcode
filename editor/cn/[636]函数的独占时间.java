...
class Solution {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        int last = -1;
        for (String log : logs) {
            String[] fun = log.split(":");
            int index = Integer.parseInt(fun[0]);
            int timestamp = Integer.parseInt(fun[2]);
            if ("start".equals(fun[1])) {
                if (!deque.isEmpty()) {
                    res[deque.peekLast()] += timestamp - last;
                }
                deque.addLast(index);
                last = timestamp;
            } else {
                int curr = deque.pollLast();
                res[curr] += timestamp - last + 1;
                last = timestamp + 1;
            }
        }
        return res;
